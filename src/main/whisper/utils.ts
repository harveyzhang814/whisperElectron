...
export class FileUtils {
  ...
  static getFileSize(filePath: string): number {
    try {
      ...
      throw ErrorHandler.createFileError(`无法获取文件大小: ${filePath}`);
    }
  }
   ...
   * 检查音频格式是否支持
   */
  static isSupportedAudioFormat(format: string): format is SupportedAudioFormat {
   ...
   * 检查输出格式是否支持
   */
  static isSupportedOutputFormat(format: string): format is SupportedOutputFormat {
    return SUPPORTED_OUTPUT_FORMATS.includes(format as SupportedOutputFormat);
  }
  ...
  static validateAudioFile(filePath: string): void {
    if (!this.fileExists(filePath)) {
      throw ErrorHandler.createFileError(`音频文件不存在: ${filePath}`);
    }

    const extension = this.getFileExtension(filePath);
    if (!this.isSupportedAudioFormat(extension)) {
      throw ErrorHandler.createFileError(
        `不支持的音频格式: ${extension}。支持的格式: ${SUPPORTED_AUDIO_FORMATS.join(', ')}`
      );
    }
    ...
    if (fileSize > maxSize) {
      throw ErrorHandler.createFileError(
        `音频文件过大: ${(fileSize / 1024 / 1024).toFixed(2)}MB，最大支持: 100MB`
      );
    }
  }
   ...
   * 创建文件流
   */
  static createFileStream(filePath: string) {
    try {
      return createReadStream(filePath);
    } catch (error) {
      throw ErrorHandler.createFileError(`无法读取文件: ${filePath}`);
    }
  }
}
...
export class ConfigUtils {
   ...
   * 验证API配置
   */
  static validateAPIConfig(config: Partial<WhisperAPIClientConfig>): void {
    const errors: string[] = [];
   ...
   * 验证模型名称
   */
  private static isValidModelName(model: string): boolean {
   ...
   */
  static mergeConfig(
    baseConfig: WhisperAPIClientConfig,
    overrideConfig: Partial<WhisperAPIClientConfig>
  ): WhisperAPIClientConfig {
    return {
      ...baseConfig,
      ...overrideConfig
   ...
   * 获取默认配置
   */
  static getDefaultConfig(): WhisperAPIClientConfig {
    return {
...
export class RequestUtils {
   ...
   * 构建API请求参数
   */
  static buildTranscribeRequest(
    filePath: string,
    config: WhisperAPIClientConfig,
   ...
   * 构建API URL
   */
  static buildAPIUrl(baseUrl: string, endpoint: string): string {
...
export class ResponseUtils {
   ...
   * 解析API响应
   */
  static parseTranscribeResponse(response: any): any {
    try {
      ...
      throw ErrorHandler.createAPIError('API响应格式无效', 0);
    }
  }
   ...
   * 验证转写响应
   */
  static validateTranscribeResponse(response: any): void {
    if (!response || typeof response !== 'object') {
      throw ErrorHandler.createAPIError('API响应格式无效', 0);
    }

    if (typeof response.text !== 'string') {
      throw ErrorHandler.createAPIError('API响应缺少文本内容', 0);
    }
  }
   ...
   * 转换为VTT格式
   */
  private static convertToVTT(response: any): string {
   ...
   * 转换为SRT格式
   */
  private static convertToSRT(response: any): string {
    if (!response.segments || !Array.isArray(response.segments)) {
      return response.text;
    }
