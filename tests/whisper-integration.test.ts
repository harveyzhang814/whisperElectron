/**
 * This module provides integration tests for the WhisperAPIClient class,
 * testing actual connections to the local Whisper Docker API.
 */
...
const testConfig: WhisperAPIClientConfig = {
  baseUrl: 'http://localhost:9000', // 默认Docker API地址
  timeout: 30000, // 30秒超时
  retryAttempts: 2,
  retryDelay: 1000,
  defaultModel: 'base',
...
async function runIntegrationTests(): Promise<void> {
  ...
  console.log('开始运行 Whisper API 集成测试...\n');
  console.log('⚠️  请确保本地 Whisper Docker API 已启动并运行在 http://localhost:9000\n');
  ...
  try {
    ...
    // 测试5: 转写测试（如果API支持）
    if (testAudioFile) {
      await runner.runTest('转写功能 - 音频文件转写', async () => {
        console.log('  开始转写测试音频文件...');
        
        let progressCount = 0;
        const result = await client!.transcribe(testAudioFile!, {}, (progress) => {
          progressCount++;
          console.log(`    进度: ${progress.progress}% - ${progress.status} - ${progress.message}`);
        });
        
        console.log(`  转写完成，结果: "${result.text}"`);
      ...
      await runner.runTest('任务管理 - 获取任务列表', () => {
        ...
        if (tasks.length === 0) {
          throw new Error('转写后应该至少有一个任务');
        }
        
        const lastTask = tasks[tasks.length - 1];
        console.log(`  最新任务状态: ${lastTask.status}`);
        console.log(`  最新任务进度: ${lastTask.progress.progress}%`);
      });
    }
  ...
  if (runner.passedCount === runner.testCount) {
    console.log('✅ 所有集成测试通过！API客户端已准备就绪。');
    console.log('📝 下一步可以:');
    console.log('  1. 集成到主进程的IPC接口');
    console.log('  2. 扩展任务管理器支持转写');
    console.log('  3. 更新UI界面显示转写状态');
  } else {
    console.log('❌ 部分测试失败，请检查:');
    console.log('  1. Whisper Docker API 是否已启动');
    console.log('  2. API地址是否正确 (当前: http://localhost:9000)');
