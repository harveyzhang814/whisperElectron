...
function createTray(mainWindow) {
    ...
    if (process.env.NODE_ENV === 'development') {
        // 在开发环境中，使用项目根目录
        iconPath = path_1.default.join(process.cwd(), 'src/assets/tray-icon.png');
    }
    else {
        // 在生产环境中，使用相对于编译后文件的路径
        iconPath = path_1.default.join(__dirname, '../assets/tray-icon.png');
    }
    ...
    try {
        ...
        const updateMenu = async () => {
            const isRecording = audio_1.audioRecorder?.getStatus().isRecording || false;
        ...
        mainWindow.webContents.on('did-finish-load', () => {
            // 监听录音状态更新
            electron_1.ipcMain.on('recording:status', () => {
                updateMenu();
            });
        });
        // 点击托盘图标显示主窗口
...
electron_1.app.whenReady().then(async () => {
    ...
    // 初始化 IPC 通信
    if (shortcutManager) {
        (0, ipc_1.initializeIPC)(shortcutManager);
    }
    // 添加 IPC 处理程序
    electron_1.ipcMain.handle('app:minimizeToTray', () => {
        mainWindow?.hide();
    });
    // 通知渲染进程主进程已就绪
    mainWindow.webContents.on('did-finish-load', () => {
