...
class ShortcutManager {
    ...
    async handleShortcut(action) {
        if (!audio_1.audioRecorder) {
            console.error('Audio recorder not initialized');
            return;
        }
        const status = audio_1.audioRecorder.getStatus();
        switch (action) {
            // 快捷键触发开始录音
            case ShortcutAction.START_RECORDING:
                if (!status.isRecording) {
                    try {
                        ...
                        const task = await taskManager_1.TaskManager.createTask(title, 'recording');
                        console.log('Created new task:', task);
                        // 开始录音
                        const result = await audio_1.audioRecorder.startRecording();
                        if (!result.success) {
                            // 如果录音失败，将任务状态改回 backlog
                            await taskManager_1.TaskManager.updateTask(task.id, { status: 'backlog' });
                            console.error('Failed to start recording:', result.error);
                        }
                        // 发送刷新事件
                        electron_1.BrowserWindow.getAllWindows()[0]?.webContents.send('task:refresh');
                        // 发送录音状态更新
                        electron_1.BrowserWindow.getAllWindows()[0]?.webContents.send('recording:status', { isRecording: true });
                    }
                    catch (error) {
            ...
            case ShortcutAction.STOP_RECORDING:
                if (status.isRecording) {
                    try {
                        ...
                        // 停止录音
                        const result = await audio_1.audioRecorder.stopRecording();
                        if (result.success) {
                            // 更新任务状态和音频路径
                            await taskManager_1.TaskManager.updateTask(currentTask.id, {
                                status: 'completed',
                            ...
                            electron_1.BrowserWindow.getAllWindows()[0]?.webContents.send('task:refresh');
                            // 发送录音状态更新
                            electron_1.BrowserWindow.getAllWindows()[0]?.webContents.send('recording:status', { isRecording: false });
                        }
                        else {
            ...
            case ShortcutAction.CANCEL_RECORDING:
                if (status.isRecording) {
                    try {
                        ...
                        // 取消录音
                        const result = await audio_1.audioRecorder.cancelRecording();
                        if (result.success) {
                            // 更新任务状态
